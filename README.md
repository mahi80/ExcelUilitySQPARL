...
- **M5 ✅** — generic **`/evals`** quality dashboard (auto-generated questions + LLM judge, any
  dataset) and an EC2 **[DEPLOY.md](docs/DEPLOY.md)**.
- **Phase-2 ✅** — **pgvector PageIndex** (semantic schema-slice retrieval for large schemas;
  full context still used for small ones) and **OpenMetadata** catalog integration (onboarding
  pushes each dataset's service → database → schema → tables → columns; OM runs as its own
  stack via [catalog/openmetadata.yml](catalog/openmetadata.yml), opt-in through `OPENMETADATA_HOST`).
